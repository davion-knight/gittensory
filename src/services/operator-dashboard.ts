...
import { computeGlobalCalibration } from "../review/global-calibration";
import type { CalibrationBinsResult } from "../../packages/gittensory-engine/src/calibration-bins";
...
export type OperatorDashboardPayload = {
  ...
  // Fleet-wide confidence-vs-outcome calibration (#2192): predicted merge confidence bucketed against realized
  // kept-rate + a recommended floor, aggregated across every project; fail-safe empty result on any read error.
  calibration: CalibrationBinsResult;
};
...
export async function buildOperatorDashboardPayload(
  ...
  const [
    ...
    // #2192: fleet-wide confidence calibration bins; reuses merged-PR confidence + reversal signals, fails safe.
    computeGlobalCalibration(env, { days: 90, nowMs: Date.now() }),
  ]);
